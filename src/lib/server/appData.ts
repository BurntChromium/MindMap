...
function rebuildEntitiesForCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  ...
  const existingEntitiesByKey = new Map(existingEntities.map((entity) => [entity.title_key, entity]));
  const nodeTitlesByKey = new Map<string, NodeEntitySource>();
  const entitiesByKey = new Map<string, EntitySeed>();
  const referencesByNodeId = new Map<string, Array<{
    title: string;
    titleKey: string;
  ...
  const deleteMentions = currentDb.prepare('DELETE FROM entity_mentions WHERE canvas_id = ?');
...
export function getNodesByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function getNodeTitlesByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function getTagsByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function getEdgesByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function getEntitiesByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function getEntityMentionsByCanvasId(canvasId: string | null, database?: SqliteDatabase) {
  if (!canvasId) {
    return [];
  }
...
export function createCanvas(
  ...
  const name = typeof input.name === 'string' && input.name.trim() ? input.name : 'New Canvas';
  const timestamp = now();
...
function getOrCreateTagId(name: string, database?: Database.Database) {
  const currentDb = getDb(database);
  const existing = currentDb.prepare('SELECT id FROM tags WHERE name = ?').get(name) as
    | { id: string; color: string | null }
    | undefined;

  if (existing) {
    if (!existing.color) {
      currentDb.prepare('UPDATE tags SET color = ? WHERE id = ?').run(getTagColor(name), existing.id);
    }
  ...
  const id = createId();
  currentDb.prepare('INSERT INTO tags (id, name, color) VALUES (?, ?, ?)').run(
    id,
    name,
    getTagColor(name)
  );
...
function replaceNodeTags(nodeId: string, tags: string[], database?: Database.Database) {
  const currentDb = getDb(database);
  const deleteNodeTags = currentDb.prepare('DELETE FROM node_tags WHERE node_id = ?');
  const insertNodeTag = currentDb.prepare('INSERT INTO node_tags (node_id, tag_id) VALUES (?, ?)');
...
function replaceEntityReferences(body: string, fromTitle: string, toTitle: string) {
...
export function updateNode(
  ...
  const node = currentDb
    ...
    .get(input.id) as { id: string; canvas_id: string; title: string; body: string | null } | undefined;
  ...
  if (typeof input.title === 'string' && hasNodeTitleConflict(getNodeTitlesByCanvasId(node.canvas_id, database), input.title, input.id)) {
  ...
  const hasTitleChange =
    typeof input.title === 'string' && normalizeNodeTitle(input.title) !== normalizeNodeTitle(node.title);
  const titleRewriteSource = node.title;
  const titleRewriteTarget = typeof input.title === 'string' ? input.title : node.title;
  const rewrittenCurrentBody = replaceEntityReferences(input.body ?? node.body ?? '', titleRewriteSource, titleRewriteTarget);
  const nextBody = input.body !== undefined || hasTitleChange ? rewrittenCurrentBody : undefined;
  ...
  const tx = currentDb.transaction(() => {
    ...
    updateNodeStmt.run(
      input.title,
      nextBody,
      typeof input.isEntity === 'boolean' ? (input.isEntity ? 1 : 0) : undefined,
      input.x,
      input.y,
      input.collapsed,
      now(),
...
export function bulkUpdateNodeTags(
  ...
  const currentDb = getDb(database);
  const touchNode = currentDb.prepare('UPDATE nodes SET updated_at = ? WHERE id = ?');
...
export function bulkUpdateNodePositions(
  ...
  const currentDb = getDb(database);
  const updateNodeStmt = currentDb.prepare('UPDATE nodes SET x = ?, y = ?, updated_at = ? WHERE id = ?');
...
export function createEdge(
  ...
  const currentDb = getDb(database);
  const id = input.id && input.id.trim() ? input.id : `e-${input.source}-${input.target}`;
...
export function pasteGraphFragment(
  ...
  const normalizedNodes = input.nodes
    .map((node) => ({
      ...
      tags: normalizeTagList(Array.isArray(node?.tags) ? node.tags.map(String) : []),
  ...
  const titleAllocator = createNodeTitleAllocator(getNodeTitlesByCanvasId(input.canvasId, database));
  ...
  const tx = currentDb.transaction(() => {
    ...
    for (const edge of normalizedEdges) {
      insertEdge.run(edge.id, input.canvasId, edge.source_node_id, edge.target_node_id);
    }
...
export function deleteGraphFragment(
  ...
  const deleteExplicitEdge = currentDb.prepare('DELETE FROM edges WHERE id = ?');
  ...
  return { success: true, removedNodes: nodeIds.length, removedEdges: edgeIds.length };
}
...
export function searchNodes(
  ...
  return searchNodesByCanvasId(input.canvasId, input.query, input.tag ?? null, database);
}
