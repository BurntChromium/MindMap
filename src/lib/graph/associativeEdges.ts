...
type EntityLike = Pick<Entity, 'id' | 'title' | 'title_key' | 'primary_node_id'>;
...
function sortEntities(left: AssociativeEdgeEntity, right: AssociativeEdgeEntity) {
...
export function buildAssociativeFlowEdges(
  ...
  for (const mention of mentions) {
    ...
    const current = mentionsByEntityId.get(mention.entity_id) ?? new Set<string>();
  ...
  for (const entity of entities) {
    const involvedNodeIds = new Set<string>(mentionsByEntityId.get(entity.id) ?? []);
