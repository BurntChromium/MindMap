...
describe('createNodePositionDebouncer', () => {
	it('keeps the latest position for each node until the debounce flushes', async () => {
		...
		const debouncer = createNodePositionDebouncer(
			applyUpdates,
			100,
			onFlushSettled,
		);
		...
		expect(debouncer.getPendingPosition('node-1', { x: 0, y: 0 })).toEqual({
			x: 34,
			y: 48,
		});
		expect(debouncer.getPendingPosition('node-2', { x: 0, y: 0 })).toEqual({
			x: 5,
			y: 6,
		});
