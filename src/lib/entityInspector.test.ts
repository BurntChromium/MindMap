...
describe('entity inspector', () => {
	it('groups a primary node with child referents', () => {
		...
		expect(entries[0].childNodes.map((node) => node.id)).toEqual([
			'node-guide',
			'node-note',
		]);
	...
	it('falls back to a flat list when no primary node exists', () => {
		...
		expect(entries[0].flatNodes.map((node) => node.id)).toEqual([
			'node-guide',
			'node-note',
		]);
	...
	it('resolves entries by title case-insensitively', () => {
		...
		expect(getEntityInspectorNodeIds(entry ?? entries[0])).toEqual([
			'node-smaug',
		]);
	});
});
