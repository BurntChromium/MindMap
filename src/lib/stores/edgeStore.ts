...
function createEdgeStore() {
	...
	return {
		...
		async load(canvasId: string) {
			...
			try {
				...
				mutationStateStore.finishLoad(
					false,
					`Failed to load edges for ${canvasId}`,
				);
				return false;
			}
		},
		...
		async create(
			...
			try {
				await appDataClient.createEdge(
					buildEdgeCreateBody({ id, canvasId, source, target }),
				);
				...
				mutationStateStore.finishWrite(
					false,
					`Create edge failed with ${String(error)}`,
				);
				console.error(error);
				return false;
			}
		...
		async remove(id: string) {
			...
			try {
				...
				mutationStateStore.finishWrite(
					false,
					`Delete edge failed with ${String(error)}`,
				);
				console.error(error);
				return false;
			}
