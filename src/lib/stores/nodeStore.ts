...
function createNodeStore() {
	...
	return {
		...
		async load(canvasId: string) {
			...
			try {
				const data = (await appDataClient.loadNodes(canvasId)) as Array<
					Node & { tags?: unknown }
				>;
				...
				mutationStateStore.finishLoad(
					false,
					`Failed to load nodes for ${canvasId}`,
				);
				return false;
			}
		},
		...
		async create(
			...
			y = 0,
			options?: Partial<
				Pick<Node, 'id' | 'title' | 'body' | 'tags' | 'collapsed' | 'is_entity'>
			>,
		) {
			const current = get(store);
			...
			const newNode: Node = {
				...
				body: options?.body ?? '',
				is_entity:
					typeof options?.is_entity === 'number' ? options.is_entity : 0,
			...
			try {
				...
				mutationStateStore.finishWrite(
					false,
					`Create node failed with ${message}`,
				);
				console.error(error);
				return false;
			}
		...
		async updateNode(partial: Partial<Node> & { id: string }) {
			...
			if (
				...
				!normalizeNodeTitle(partial.title)
			) {
			...
			if (
				typeof partial.title === 'string' &&
				hasNodeTitleConflict(currentNodes, partial.title, partial.id)
			) {
			...
			try {
				...
				mutationStateStore.finishWrite(
					false,
					`Update node failed with ${message}`,
				);
				console.error(error);
				return false;
			}
		...
		async updateNodeTags(updates: NodeTagUpdate[]) {
			...
			const nextTagsById = new Map(
				updates.map((update) => [update.id, [...update.tags]]),
			);
			const previousTagsById = new Map(
				updates.map((update) => [
					update.id,
					previous.nodes.get(update.id)?.tags
						? [...previous.nodes.get(update.id)!.tags]
						: [],
				]),
			);
			...
			try {
				...
				mutationStateStore.finishWrite(
					false,
					`Bulk tag update failed with ${String(error)}`,
				);
				console.error(error);
				return false;
			}
		...
		async updateNodePositions(updates: NodePositionUpdate[]) {
			...
			const previousPositionsById = new Map(
				updates.map((update) => {
					...
					return [
						update.id,
						node ? { x: node.x, y: node.y } : { x: update.x, y: update.y },
					];
				}),
			);
			mutationStateStore.beginWrite();
			...
			try {
				...
				mutationStateStore.finishWrite(
					false,
					`Bulk node position update failed with ${String(error)}`,
				);
				console.error(error);
				return false;
			}
		...
		async remove(id: string) {
			...
			try {
				...
				const refreshCanvasId =
					removedNode?.canvas_id ?? previous.activeCanvasId;
				...
				mutationStateStore.finishWrite(
					false,
					`Delete node failed with ${String(error)}`,
				);
				console.error(error);
				return false;
			}
