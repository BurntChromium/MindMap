...
function seedDatabase() {
  ...
  const tx = db.transaction(() => {
    insertCanvas.run(fixture.canvasId, 'Benchmark Canvas', 1_700_000_000_000, 1_700_000_000_000);
    ...
    for (const edge of fixture.serverEdges) {
      insertEdge.run(edge.id, edge.canvas_id, edge.source_node_id, edge.target_node_id);
    }
...
describe('server query benchmarks', () => {
  ...
  bench('search active canvas by keyword', () => {
    graphData.searchNodesByCanvasId(fixture.canvasId, fixture.searchQuery, null);
  });
