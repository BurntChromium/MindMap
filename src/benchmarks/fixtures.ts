...
function buildServerTagLinks(
	nodes: ReturnType<typeof buildAppNodes>,
	tags: string[],
) {
	const tagIdByName = new Map(
		tags.map((name, index) => [name, `tag-${String(index).padStart(2, '0')}`]),
	);
...
function buildTagColors(tags: string[]) {
	return Object.fromEntries(
		tags.map((name) => [name, getTagColor(name)]),
