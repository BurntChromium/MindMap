...
export function GET() {
	...
	} satisfies AppDataDatabaseSettings);
}
...
export async function PATCH({ request }) {
	...
	if (!databaseFileName.trim()) {
		return json(
			{ success: false, error: 'Database file name is required.' },
			{ status: 400 },
		);
	}
		...
		return json(
			{
				...
				error:
					error instanceof Error
						? error.message
						: 'Failed to update the database file name.',
			},
			{ status: 400 },
		);
	}
