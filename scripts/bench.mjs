...
async function main() {
  ...
  try {
    ...
    if (command === 'compare') {
      ...
      const comparison = compareBenchmarkReports(baselineReport, currentReport, budgetPct);

      if (comparison.regressions.length || comparison.missing.length || comparison.extra.length) {
        ...
        for (const entry of comparison.missing) {
          console.error(`Missing benchmark: ${entry.filepath} > ${entry.groupName} > ${entry.name}`);
        }

        for (const entry of comparison.extra) {
          console.error(`New benchmark: ${entry.filepath} > ${entry.groupName} > ${entry.name}`);
        }
