...
function isDiagnosticBenchmark(entry) {
  return DIAGNOSTIC_BENCHMARK_PATTERNS.some((pattern) => pattern.test(entry.name));
}
...
export function compareBenchmarkReports(baselineReport, currentReport, budgetPct = DEFAULT_BUDGET) {
  const baselineEntries = flattenReport(baselineReport);
  const currentEntries = flattenReport(currentReport);
  const baselineMap = new Map(
    baselineEntries.filter((entry) => !isDiagnosticBenchmark(entry)).map((entry) => [entryKey(entry), entry])
  );
  const currentMap = new Map(
    currentEntries.filter((entry) => !isDiagnosticBenchmark(entry)).map((entry) => [entryKey(entry), entry])
  );
  ...
  for (const key of allKeys) {
    ...
    const hzDeltaPct = baseline.hz === 0 ? 0 : ((current.hz - baseline.hz) / baseline.hz) * 100;
  ...
  return {
    ...
    missing,
    extra,
    passed: regressions.length === 0 && missing.length === 0 && extra.length === 0
  };
}
...
export async function loadAndNormalizeBenchmarkReport(filePath, rootDir = process.cwd()) {
...
export function parseBenchmarkArgs(argv) {
  ...
  for (let index = 3; index < argv.length; index += 1) {
    ...
    if (value.startsWith('--baseline=')) {
      result.baselinePath = value.slice('--baseline='.length) || result.baselinePath;
      continue;
    }
